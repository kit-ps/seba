...
class Datapoint_classificationUtility(Classification, AbstractMotionUtility):
    ...
    Parameters:
        ...
        - (string) attribute: The attribute of the datapoints which is used for classification
        - (dict)   attribute_conversion: A dict which converts attributes to its class label
    """
    ...
    def validate_config(self):
        ...
        if "attribute" not in self.config:
            self.log.error("Missing parameter value for attribute")
    ...
    def classify_point(self, mocap):
        ...
        label = getattr(mocap, self.config["attribute"])
        ...
        labels = self.label_encoder.inverse_transform(self.trained_clf.classes_)

        for i in range(len(results)):
            rs.add_recognized(labels[i], dist=1 - results[i])
